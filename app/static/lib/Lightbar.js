import { emdrGetConstant, MovementDirection } from "./utils";
...
Lightbar.prototype._isAtMidpoint = function () {
	...
	if (
		parsedMarginLeft > lowerTolerance &&
		parsedMarginLeft < upperTolerance
	) {
		// within midpoint
		return true;
	}
...
Lightbar.prototype._getAveragePxDelta = function () {
	return (
		this.data.frames.reduce((acc, frame) => acc + frame, 0) /
		this.data.frames.length
	);
};
...
Lightbar.prototype._doBounce = function (timestamp) {
	...
	const speed = this.data.isStopping
		? this.data.stoppingSpeed
		: this.data.speed;
	...
	switch (this.data.movementDirection) {
		case "left":
			this._handleMoveLeft(parsedMarginLeft, pxDelta);
			break;
		case "right":
			this._handleMoveRight(parsedMarginLeft, pxDelta);
			break;
	}
...
Lightbar.prototype._handleMoveRight = function (currentMarginLeft, pxDelta) {
	...
	if (currentMarginLeft === this.data.maxMarginLeft) {
		...
		/**
			...
			*/
		const inBoundsMarginLeft =
			this.data.maxMarginLeft -
			(nextMarginLeft - this.data.maxMarginLeft);
		this.lightElement.style.marginLeft = inBoundsMarginLeft + "px";
		this._changeDirection(MovementDirection.Left);
	} else {
		this.lightElement.style.marginLeft = nextMarginLeft + "px";
